...
<div class="form-aportes">
  ...
  <!-- Desplegable Obligatorio / Opcional -->
...
<script>
  ...
  const ramosPorAno = {
    ...
    "3er Año": ["Analisis Real I", "Optimizacion I","Probabilidades","Mecanica de Fluidos","Analisis Numerico II","Analisis Real II","Electromagnetismo","Mecanica de Materiales","Optimizacion II","Inferencia Estadistica"],
    ...
    "6to Año": ["Sistemas Lineales Dinamicos","Introduccion a Ciencias Ambientales","Gestion de Empresas"],
    "Electivos": ["Numerico","Discreta","Opti","Estadistica","Libre"],
    "Otros": ["Licenciatura en Matematicas","Informatica","Extra"]
  };

  // Función para normalizar texto (quita tildes, limpia espacios repetidos y capitaliza cada palabra)
  ...
  function actualizarVisibilidadCampos() {
    ...
    // Obligatorio u Opcional solo si es Electivos y NO es "Libre"
    ...
    // Nombre de ramo libre para Electivos u Otros
  ...
  function procesarArchivo(file) {
    ...
    // Validaciones
    if (!anoSeleccionado || !ramoSeleccionado || !inputAnoMaterial.value) {
      mostrarError('Faltan datos por llenar.');
      return;
    }

    if (containerTipoElectivo.style.display === 'flex' && !selectTipoElectivo.value) {
      mostrarError('Falta indicar si es Obligatorio u Opcional.');
      return;
    }
    ...
    let partesRuta = [anoSeleccionado];

    if (anoSeleccionado === "Electivos") {
      partesRuta.push(ramoSeleccionado); // Ej: "Numerico" o "Libre"
      
      if (ramoSeleccionado !== "Libre") {
        partesRuta.push(selectTipoElectivo.value); // Ej: "Obligatorio"
      }
      
      partesRuta.push(normalizarTexto(inputNombreRamo.value)); // Ej: "Elementos Finitos"

    } else if (anoSeleccionado === "Otros") {
      partesRuta.push(ramoSeleccionado); // Ej: "Informatica"
      partesRuta.push(normalizarTexto(inputNombreRamo.value)); // Ej: "Sistemas Operativos"

    } else {
      // 1er a 6to Año: Ruta estándar -> [Año, Ramo]
      partesRuta.push(ramoSeleccionado);
    }
    ...
    reader.onload = function(event) {
      ...
      // Enviamos la ruta completa en formato JSON para que Apps Script la cree nivel por nivel
      formData.append('rutaCarpetas', JSON.stringify(partesRuta));
